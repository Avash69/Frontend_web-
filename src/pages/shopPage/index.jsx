...
const Index = () => {
  ...
  // Sort products on load and when sorting criteria changes
  useEffect(() => {
    handleSorting(dispatch, sortingCriteria, allProductsData, NoOfProductsPerPage, currentPageNo, location.pathname);
  }, [dispatch, sortingCriteria, allProductsData, currentPageNo, location.pathname]);

  // Paginate filtered data when current page or filtered data changes
  useEffect(() => {
    handlePaginationProductsPage(
      dispatch,
      NoOfProductsPerPage,
  ...
  return (
    <section className="lg:grid lg:grid-cols-[250px_1fr_1fr_1fr] lg:grid-rows-[auto_1fr_auto] min-h-screen">
      ...
      >
        <div className="flex gap-[4px] items-center text-base">
          <IoIosArrowBack />
          <li onClick={() => navigate("/")} className="hover:underline capitalize cursor-pointer" role="link" tabIndex={0} onKeyDown={e => e.key === "Enter" && navigate("/")}>
            Home
          </li>
          <IoIosArrowBack />
          <span>Shop</span>
        ...
        {isLoading ? (
          ...
          <>
            <div className="lg:flex lg:justify-between lg:items-start px-4">
              ...
              {/* Sorting Section */}
              <section className="w-[65%] tablet:w-[40%] md:w-[30%] bg-white lg:order-1 laptop:w-[17%] lg:w-[30%] ml-[4%] tablet:ml-[6%] mb-20 flex flex-col gap-2 lg:ml-0 lg:max-w-[262px] rounded shadow-md p-4">
                ...
                <ul
                  className={`hidden flex-col bg-white rounded-md shadow-md py-4 gap-4 z-[200] px-[10%] sticky top-0 left-0 right-0 mb-[-16.5rem] lg:mb-[-15.5rem] transition duration-700 ease-in-out sorting-lists ${
                    sortingCriteria !== "Default: Latest" ? "bg-lightPrimaryColor text-white" : ""
                  }`}
                    ...
                    <li
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          ...
                          e.currentTarget.parentElement.classList.remove("active-sorting-lists");
                        }
                      }}
                    >
                      {criteria}
            ...
            {placeholderOfproductsDataCurrentlyRequested.length > 0 ? (
              <>
                <section className="grid grid-cols-1 tablet:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 w-[92%] mx-auto items-center justify-center gap-[4rem] tablet:justify-between tablet:w-[88%] md:justify-between tablet:gap-y-12 md:gap-y-12 md:gap-[5%] tablet:gap-[4%]">
                  {productsDataForCurrentPage.map((productsData, index) => (
                ...
                <div className="fixed right-[7%] bottom-[7%] lg:hidden z-[1000] flex flex-col items-center">
                  <BiFilter
                    className="w-16 h-16 bg-darkPrimaryColor shadow-lg shadow-[rgba(0,0,0,0.2)] fill-secondaryColor cursor-pointer"
                    onClick={() => setIsFilterBySectionOpen(true)}
                  ...
                  />
                  <span className="mt-1 w-20 px-2 py-1 bg-gray-700 rounded-lg text-center text-white text-sm shadow-lg">
                    Filter Products
                  </span>
                </div>
              </>
            ) : (
              <h1 className="text-center text-[28px] md:text-[32px] lg:text-[36px] mt-20">No product matches found.</h1>
            )}
          </>
        )}
      </main>
