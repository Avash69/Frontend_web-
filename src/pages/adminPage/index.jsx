...
export const Index = () => {
  ...
  useEffect(() => {
    const tabPath = location.pathname.replace("/administrator/", "");
    const tabName = tabPath.includes("management") ? tabPath : tabPath.replace("-", " ");
    setCurrentTabInAdminPage(tabName || "dashboard");
  }, [location.pathname]);
  ...
  if (checkingAdminStatusLoader) return <FullpageSpinnerLoader />;

  return (
    <>
      {/* Top Breadcrumb Nav */}
      <div className="mt-12 tablet:px-[6%] w-full h-[54px] bg-neutralColor text-secondaryColor xl:px-[4%] px-[4%] lg:px-[2%] flex items-center justify-between font-bold font-RobotoCondensed">
        <div className="flex gap-2 items-center text-base">
          <IoIosArrowBack />
          <li onClick={() => navigate("/")} className="hover:underline cursor-pointer capitalize">Home</li>
          <IoIosArrowBack />
          <li onClick={() => navigate("/shop")} className="hover:underline cursor-pointer capitalize">Shop</li>
          <IoIosArrowBack />
          <span>Admin</span>
        </div>
      </div>
      ...
      {/* Sidebar and Logout */}
      <div className="flex mx-[4%] justify-between items-start mt-16 tablet:mx-[6%]">
        <article className="w-[50%] tablet:w-[35%] lg:w-[30%] md:w-[30%] bg-white max-w-[264px] mb-16 flex-col flex gap-2">
          {/* Dropdown Toggle */}
          <div
            className="flex justify-between h-14 rounded-md shadow-md items-center px-[10%] cursor-pointer bg-lightPrimaryColor text-white"
            onClick={() => setDropdownOpen(!dropdownOpen)}
          >
            <h2 className="capitalize">{currentTabInAdminPage}</h2>
            <IoIosArrowDown className="w-6 h-6" />
          </div>
          ...
          <div
            ref={dropdownRef}
            className={`flex-col transition-all duration-300 ease-in-out overflow-hidden ${
              dropdownOpen ? "flex" : "hidden"
            } rounded-md shadow-md py-4 gap-4 bg-lightPrimaryColor text-white px-[10%]`}
          >
            <Link to="dashboard">
              <li data-tabpath="Dashboard" className="hover:underline cursor-pointer">Dashboard</li>
            </Link>
            <Link to="product-Management">
              <li data-tabpath="Product management" className="hover:underline cursor-pointer">Product Management</li>
            </Link>
            <Link to="user-Management">
              <li data-tabpath="Users management" className="hover:underline cursor-pointer">Users Management</li>
            </Link>
            <Link to="admin-Management">
              <li data-tabpath="Admins" className="hover:underline cursor-pointer">Admins</li>
            </Link>
          </div>
        </article>
        ...
        {/* Logout Button */}
        <button
          className="text-center w-[100px] h-9 hover:opacity-100 bg-lighterPrimaryColor text-primaryColor cursor-pointer shadow-md rounded-md flex items-center justify-center"
          onClick={logoutBtnClick}
        >
          <span>Logout</span> &nbsp;
      ...
      {/* Welcome Message */}
      <h2 className="font-RobotoSlab text-3xl font-bold my-20 w-full xl:pl-[4%] tablet:pl-[6%] pl-[4%] lg:pl-[2%]">
        Welcome, Admin
      </h2>
