...
export const OrderSummary = ({ setTotalAmountToBePaid }) => {
  ...
  // Filter out products with invalid quantity
  const filteredCart = cart.filter((product) => product.quantity > 0);
  ...
  useEffect(() => {
    const total = totalProductPrice + productTotalQuantity * shippingMethodValue;
    setTotalAmountToBePaid(Number(total.toFixed(2)));
  }, [totalProductPrice, shippingMethodValue, productTotalQuantity, setTotalAmountToBePaid]);
    ...
    <section className="mt-20 mb-20 lg:mb-0 w-[92%] tablet:w-[88%] mx-auto lg:mx-0 bg-white border-2 border-LightSecondaryColor py-8 lg:order-2 lg:basis-[40%] xl:basis-[35%]">
      ...
      <div className="flex flex-col gap-4 w-[90%] max-w-[500px] mx-auto">
        {filteredCart.map((item) => (
          <article
            key={item._id}
            className="flex gap-4 justify-between border-b pb-4 border-LightSecondaryColor"
          >
            <div className="w-[40%] tablet:w-[45%] md:w-[45%] h-[110px] tablet:h-[180px] md:h-[160px] bg-neutralColor flex items-center justify-center">
              <img
                src={item.image}
                alt={item.title}
                className="rounded-sm object-contain h-auto max-h-[90%] max-w-[90%] w-full"
              />
            </div>
            <div className="flex flex-col gap-2 w-[30%] text-[16px]">
              <h2 className="text-[18px] font-normal font-RobotoSlab line-clamp-2">{item.title}</h2>
      ...
      <div className="pt-4 flex flex-col gap-4 border-t-2 border-LightSecondaryColor mt-20 w-full">
        <div className="flex justify-between mx-[5%] border-b pb-4 border-LightSecondaryColor">
          <h2 className="font-normal text-[18px] md:text-[20px]">SubTotal</h2>
          <span className="text-lg tracking-wide">${totalProductPrice.toFixed(2)} USD</span>
        </div>
        ...
        <div className="flex justify-between mx-[5%] border-b pb-4 border-LightSecondaryColor">
          <div className="flex flex-col gap-2">
            <h2 className="font-normal text-[18px] md:text-[20px]">Shipping Option</h2>
            <span className="text-lg font-RobotoCondensed capitalize">
              {shippingMethod || "N/A"} rate
            </span>
          </div>
          <span className="text-lg tracking-wide">
            ${(shippingMethodValue * productTotalQuantity).toFixed(2)} USD
          </span>
        </div>

        <div className="flex justify-between mx-[5%]">
          <h2 className="font-bold text-[20px] md:text-[24px]">Total</h2>
          <h2 className="font-bold text-[20px] md:text-[24px] tracking-wide">
