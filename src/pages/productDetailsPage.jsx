...
// import { ProductLoader } from "../components/loaders/productLoader.jsx";
...
//           <div
//             className={`absolute p-3 bg-[#ffffff] shadow-[0px_2px_8px_0px_#00000085] ease-in transition-colors cursor-pointer duration-300 rounded-[50%] top-[5%] right-[5%] z-[100] ${isWishlisted && "bg-primaryColor"
//               }`}
//           >
//             <FiHeart
//               className={`w-6 h-6 ${isWishlisted && "fill-primaryColor duration-200 ease-linear transition-colors stroke-white"
//                 }`}
...
export const ProductDetailsPage = () => {
  ...
  // Gather sub-categories into an array
  let subCategoriesArr = [];
  for (let key in categories) {
    if (Array.isArray(categories[key]) && categories[key].length > 0) {
      subCategoriesArr.push(...categories[key]);
    }
  }
  ...
  // Check if product is in wishlist and cart to reflect UI state
  useEffect(() => {
    isProductInWishlistFn(_id, setIsWishlisted, wishlist);
  }, [wishlist, _id]);
  ...
  const buyNowFn = () => {
    ...
    // Add or update cart and then navigate to checkout
    handleCartModification(_id, dispatch, productQuantityInCart, isProductInCart);
    navigate("/checkout");
  };
  ...
  return (
    <motion.div
      ...
      className="w-full"
    >
      <div className="mt-12 w-full h-[54px] bg-neutralColor text-secondaryColor xl:px-[4%] px-[4%] lg:px-[2%] flex items-center justify-between font-bold tablet:px-[6%] font-RobotoCondensed lg:col-span-full lg:row-span-1">
        <div className="flex gap-[4px] items-center text-base">
          <IoIosArrowBack />
      ...
      <section className="my-20 mb-32 w-[92%] mx-auto gap-6 flex flex-col lg:flex-row lg:gap-2 md:justify-between tablet:w-[88%] lg:w-[96%]">
        <div className="w-full lg:mx-0 lg:basis-[50%] lg:h-max min-h-[320px] tablet:min-h-[450px] md:min-h-[500px] h-auto mx-auto bg-neutralColor relative flex justify-center items-center">
          <img src={image} alt={title || "Product image"} className="w-auto max-w-[99%] h-auto object-cover" />
          <div
            className={`absolute p-3 bg-white shadow-[0px_2px_8px_0px_#00000085] ease-in transition-colors cursor-pointer duration-300 rounded-full top-[5%] right-[5%] z-[100] ${
              isWishlisted ? "bg-primaryColor" : ""
            }`}
          >
            <FiHeart
              className={`w-6 h-6 ${isWishlisted ? "fill-primaryColor stroke-white transition-colors duration-200 ease-linear" : ""}`}
              onClick={() => handleWishlistModification(_id, dispatch)}
              aria-label={isWishlisted ? "Remove from wishlist" : "Add to wishlist"}
              role="button"
              tabIndex={0}
              onKeyPress={(e) => e.key === "Enter" && handleWishlistModification(_id, dispatch)}
            />
          </div>
        </div>
        <div className="lg:basis-[40%] mt-16 lg:mt-0 flex flex-col gap-6">
          ...
          {discountPercentValue > 0 ? (
            <div className="flex gap-2">
              <h3 className="font-bold text-[24px] md:text-[28px] tracking-[1px]">${discountedPrice.toFixed(2)} USD</h3>
              <h3 className="line-through tracking-[1px] text-[20px] md:text-[24px]">${price.toFixed(2)} USD</h3>
            </div>
          ) : (
            <h3 className="font-bold text-[24px] md:text-[28px] tracking-[1px]">${price.toFixed(2)} USD</h3>
          ...
          <div className="flex gap-1 items-end">
            <h3 className="font-bold tracking-[0.5px] text-[20px]">Availability :</h3>
            <span className="text-primaryColor tracking-[0.7px] text-[18px]">
              {stock <= 0 ? "Out of stock" : <strong>{stock}</strong>} {stock > 0 && " left in stock"}
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setProductQuantityInCart(val > 0 ? val : 1);
              }}
          ...
          <div>
            <h3 className="font-bold text-[20px] tracking-[0.5px]">Sub-Categories :</h3>
            <div className="flex flex-wrap gap-2 font-medium text-[18px]">
              {subCategoriesArr.length > 0
                ? subCategoriesArr.map((category, i) => (
                    <span key={`${category}-${i}`} className="capitalize">
                      {category}
                      {i !== subCategoriesArr.length - 1 ? "," : ""}
                    </span>
                  ))
                : "No sub-categories"}
            </div>
          ...
          <div className="flex gap-6 flex-wrap">
            <motion.button
              ...
              whileHover={{ backgroundColor: "#000000", borderWidth: "0px", color: "#ffffff" }}
              transition={{ duration: 0.4 }}
              className="text-secondaryColor basis-[100%] tablet:basis-[45%] md:basis-[35%] lg:basis-[40%] bg-transparent border-[1px] border-secondaryColor font-semibold w-full h-[50px]"
              onClick={handleAddAndRemoveItemInCartFn}
            >
              {isProductInCart ? "Remove from Cart" : "Add to Cart"}
            </motion.button>
            ...
            <motion.button
              ...
              whileTap="click"
              variants={primaryBtnVariant}
              className="text-white bg-primaryColor font-semibold w-full basis-[100%] lg:basis-[40%] tablet:basis-[45%] md:basis-[35%] h-[50px] block"
              onClick={buyNowFn}
            >
              Buy Now
            </motion.button>
          ...
          <div className="flex-col flex gap-4">
            ...
            <div className="flex flex-col gap-2">
              <p className="leading-[150%]">
                <span className="font-semibold text-[18px]">Standard shipping</span> (Delivery in 7-10 working days) - takes $7.00 USD
                for each product
              </p>
              <p className="leading-[150%]">
                <span className="font-semibold text-[18px]">Express shipping</span> (Delivery in 5-7 working days) - takes $7.00 USD for each
                product
              </p>
              <p className="leading-[150%]">
                <span className="font-semibold text-[18px]">Free shipping</span> (Delivery in 11-13 working days) - takes $0.00 USD for each product
              </p>
            </div>
          </div>
        </div>
